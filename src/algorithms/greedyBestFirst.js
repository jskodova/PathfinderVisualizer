export function greedyBestFirst(grid, startNode, finishNode) {
    const openSet = [];
    const closedSet = [];
    startNode.hScore = heuristic(startNode, finishNode); // Initialize hScore as the heuristic value
    openSet.push(startNode);

    while (openSet.length > 0) {
      const currentNode = getLowestHScoreNode(openSet); // Use hScore to get the lowest value node
  ...
  function getLowestHScoreNode(nodes) {
    ...
    for (let i = 1; i < nodes.length; i++) {
      if (nodes[i].hScore < lowestNode.hScore) { // Use hScore for comparison
        lowestNode = nodes[i];
      }
    }
    return lowestNode;
